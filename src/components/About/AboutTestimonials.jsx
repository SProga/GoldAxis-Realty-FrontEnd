...
export default function AboutTestimonials({ items = [], eyebrow, title }) {
  ...
  const controlClass =
    "rounded-full p-3 text-muted transition-colors hover:text-primary focus-visible:outline-2 focus-visible:outline-primary";
      ...
      <AboutReveal className="mx-auto max-w-[1050px] text-center">
        ...
        <div className="mt-7 flex items-center justify-center gap-3 md:gap-12">
          ...
          >
            ...
            >
              <blockquote className="text-base italic leading-7 text-muted">
                “{current.description}”
              </blockquote>
          ...
          <div className="mt-6 flex justify-center">
            {items.map((item, i) => (
              <button
                ...
                onClick={() => setIndex(i)}
                className=" flex h-4 w-6 items-center justify-center rounded-full focus-visible:outline-2 focus-visible:outline-primary cursor-pointer"
              >
                <span
                  className={`h-2.5 w-2.5 rounded-full ${i === activeIndex ? "bg-primary" : "bg-foreground/25"}`}
                />
              </button>
            ))}
          </div>
